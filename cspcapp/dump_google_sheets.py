...
def update_form_info(reuqest: WSGIRequest) -> HttpResponse:
    ...
    values = [[
        ...
        f"{i.teacher_person.person_name_txt} {i.teacher_person.person_father_name_txt or ''}. День недели: {i.get_week_days_str}"]
        for i in CourseElement.objects.all() if not i.deleted
    ]
    body = {'values': values}
